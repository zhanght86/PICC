...
Rem 打的增量rar包和list_insured.txt都放到工作空间的upgrade_working目录下
Set  upgrade_working_path=%update_package_root%\temp


rem 修改list_insured.txt文件，使其可为打增量rar服务
%script_disk%
Cd %replace_script_path%
cscript replace.vbs "%upgrade_working_path%\list_insured.txt" ".java" ".class"
cscript replace.vbs "%upgrade_working_path%\list_insured.txt" "Modified" ""

cscript replace.vbs "%upgrade_working_path%\list_insured.txt" "src/main/java" "target/insured/WEB-INF/classes" 

cscript replace.vbs "%upgrade_working_path%\list_insured.txt" "Added" ""

Rem  --------------2.  在webapp下面，打jar包---------------------
%disk%
cd %webapps_path%
jar -cvfM %upgrade_working_path%\%module%.rar @%upgrade_working_path%\list_insured.txt 
...
rd /s /q %module%
pause
call E:\中科软科技股份有限公司\移动脚本命令\move_insured.bat